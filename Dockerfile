...
RUN /usr/lib/graalvm/bin/native-image --no-server -cp build/libs/completeaws-*-all.jar -H:Name=completeaws -H:Class=example.micronaut.BookLambdaRuntime
RUN chmod 777 bootstrap
RUN chmod 777 completeaws
RUN zip -j function.zip bootstrap completeaws
EXPOSE 8080
